...
import edu.sjsu.cmpe.library.message.broker.Producer;
...
public class BookResource {
	...
	Book book = bookRepository.getBookByISBN(isbn.get());
	// if the status is set to lost it is going to send the message to queue	
	if(status.toString() == "lost")
	{
		if(book.getStatus().toString() != "lost")
		{
			...
			try{
				producer.sendQueueMessage(isbn.get(),configuration);
			}
